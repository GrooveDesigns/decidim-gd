...
module Decidim
  module Proposals
    ...
    module ProposalVotesHelper
      ...
      def no_login_vote_key
        return "#{:no_login_vote}" unless request.subdomain
        "#{:no_login_vote_}#{request.subdomain}"
      end

      def no_login_voted_proposal_ids
        return [] unless cookies[no_login_vote_key].present?
        return cookies[no_login_vote_key].split("&") unless cookies[no_login_vote_key].is_a?(Array)
        cookies[no_login_vote_key]
      end

      def already_nologin_voted?(proposal)
        return true if no_login_voted_proposal_ids.include?(proposal.id.to_s)
        false
      end

      def create_nologin_vote(proposal)
        ids = no_login_voted_proposal_ids
        return if ids.include?(proposal.id.to_s)
        ids.push(proposal.id.to_s)
        cookies.permanent[no_login_vote_key] = ids
      end

      def destroy_nologin_vote(proposal)
        return if no_login_voted_proposal_ids.empty?
        ids = no_login_voted_proposal_ids
        ids.delete(proposal.id.to_s)
        cookies.permanent[no_login_vote_key] = ids
      end

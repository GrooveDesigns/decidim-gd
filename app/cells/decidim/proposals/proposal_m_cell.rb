...
module Decidim
  module Proposals
    ...
    class ProposalMCell < Decidim::CardMCell
      ...
      def votes_count
        with_tooltip t("decidim.proposals.proposals.votes_count.count.explanation") do
          "#{icon("bullhorn", class: "icon--small")} #{progress_bar_progress}#{t("decidim.proposals.proposals.votes_count.count.other")}"
        end
      end

      def progress_bar_progress
        model.total_votes_count || 0
      end

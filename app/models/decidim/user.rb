...
module Decidim
  ...
  class User < UserBaseEntity
    ...
    validates :real_name, presence: true, unless: -> { deleted? || admin? }
    validates :locale, inclusion: { in: :available_locales }, allow_blank: true
    validates :tos_agreement, acceptance: true, allow_nil: false, on: :create
    validates :tos_agreement, acceptance: true, if: :user_invited?
    validates :privacy_policy_agreement, acceptance: true, allow_nil: false, on: :create
    validates :privacy_policy_agreement, acceptance: true, if: :user_invited?
    validates :email, :nickname, uniqueness: { scope: :organization }, unless: -> { deleted? || managed? || nickname.blank? }
    validates :zipcode, presence: true, format: { with: /[0-9]{3}-?[0-9]{4}/ }, unless: -> { admin? }
    validates :birth_year, presence: true, numericality: { only_integer: true, greater_than: 0 }, unless: -> { admin? }

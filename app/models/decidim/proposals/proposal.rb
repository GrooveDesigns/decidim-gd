  ...
  module Proposals
    ...
    class Proposal < Proposals::ApplicationRecord
      ...
      def total_votes_count
        proposal_votes_count + proposal_no_login_votes_count
      end

      def increment_nologin_votes_count
        increment!(:proposal_no_login_votes_count, 1)
      end

      def decrement_nologin_votes_count
        return if self.proposal_no_login_votes_count == 0
        decrement!(:proposal_no_login_votes_count, 1)
      end
      ...
      ## Decidim::ActionLog#visible_for?メソッド内でコールされる、任意条件でActionLogの可視性をコントロールするメソッド
      def invisible_action_log?
        withdrawn?
      end

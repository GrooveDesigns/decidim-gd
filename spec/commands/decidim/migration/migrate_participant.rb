...
RSpec.describe Decidim::Migration::MigrateParticipant, type: :command do
      ...
      describe 'organization1, 2共に含む場合' do
        ...
        it do
          ...
          expect(resources.map { |u| m.send(:parse_from, u) }).to eq [
                                                                       [Digest::SHA256.hexdigest(email1_organization1_user.email), organization1.id],
                                                                       [Digest::SHA256.hexdigest(email1_organization1_user.email), organization2.id],
                                                                       [Digest::SHA256.hexdigest(email2_organization1_user.email), organization1.id],
                                                                       [Digest::SHA256.hexdigest(email3_organization2_user.email), organization2.id],
                                                                     ]
        end
      end
